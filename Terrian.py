...
def export_scene_to_obj(path, terrain, terrain_tex_ids, terrain_weights,
    ...
    tris = terrain.tris
    uvs = np.stack([verts[:, 0] / max(6.0, 1e-6), verts[:, 2] / max(6.0, 1e-6)], axis=1)

    # dominant splat layer per-vertex decides which single material the
    # face is assigned to (OBJ has no per-vertex blend weights) - visually
    # close enough for a Blender starting point, and every layer's baked
    # texture is still written out for manual re-blending if desired
    dominant = np.argmax(terrain_weights, axis=1)
    layer_names = ["grass", "dirt", "rock", "sand"]
    for key in layer_names:
        add_material_textured("terrain_" + key, terrain_tex_ids.get(key))
    ...
    # group triangles by dominant layer so each material only needs one
    # usemtl switch instead of one per face
    tri_layer = dominant[tris[:, 0]]  # use the first vertex's layer per tri
    for layer_idx, key in enumerate(layer_names):
        face_idx = np.nonzero(tri_layer == layer_idx)[0]
        if len(face_idx) == 0:
            continue
        obj_lines.append("usemtl terrain_" + key)
        for fi in face_idx:
            a, b, c = tris[fi]
            obj_lines.append(
    ...
    ROAD_PROP_COLORS = {"fence": (0.42, 0.30, 0.16), "lamp": (0.20, 0.20, 0.22)}
    for ri, road in enumerate(roads):
        ...
        if instances:
            ...
            for prop_type, inst_list in by_type.items():
                try:
                    proto_v, proto_n, proto_tris = _build_road_prop_prototype_geometry(prop_type)
                except Exception:
                    continue  # custom-imported prop types skipped (no python-side prototype)
                mat_name = "prop_{}".format(prop_type)
                add_material_flat(mat_name, ROAD_PROP_COLORS.get(prop_type, (0.4, 0.4, 0.4)))

                obj_lines.append("o {}_{}".format(road_name, prop_type.capitalize()))
                inst_v_count = 0
                inst_vn_count = 0
                all_faces = []
...
def _build_road_prop_prototype_geometry(prop_type):
    ...
    if prop_type == "fence":
        ...
        for i in range(segs):
            ...
            n0 = (math.cos(a0), 0.0, math.sin(a0))
            n1 = (math.cos(a1), 0.0, math.sin(a1))
            push_tri((x0, 0.0, z0), (x0, post_h, z0), (x1, 0.0, z1), n0, n0, n1)
            push_tri((x1, 0.0, z1), (x0, post_h, z0), (x1, post_h, z1), n1, n0, n1)
        rail_len, rail_thick = 0.9, 0.04
        n = (0.0, 1.0, 0.0)
        for rail_y in (0.30, 0.55):
        ...
        for i in range(segs):
            ...
            n0 = (math.cos(a0), 0.0, math.sin(a0))
            n1 = (math.cos(a1), 0.0, math.sin(a1))
            push_tri((x0, 0.0, z0), (x0, pole_h, z0), (x1, 0.0, z1), n0, n0, n1)
            push_tri((x1, 0.0, z1), (x0, pole_h, z0), (x1, pole_h, z1), n1, n0, n1)
        r = 0.16
        for p0, p1, p2 in _sphere_triangles(4, 6):
            pts, ns = [], []
            for p in (p0, p1, p2):
