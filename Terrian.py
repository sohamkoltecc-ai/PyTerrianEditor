...
def browse_for_save_file(title, default_ext, filetypes):
    """Native "Save As" dialog via Tkinter, mirroring browse_for_file's
    fallback behavior. Returns (True, path) if the dialog opened (path is
    "" if cancelled), or (False, "") if Tkinter is unavailable, in which
    case the caller should fall back to a text-entry field for the path."""
    try:
        import tkinter as tk
        from tkinter import filedialog
        root = tk.Tk()
        ...
        print("Native save dialog unavailable ({}); type an export path instead.".format(e))
        return False, ""
...
# ---------------------------------------------------------------------------
# Scene export (single combined .obj usable directly in Blender)
# ---------------------------------------------------------------------------
#
# Exports the terrain, every foliage instance, and every road (mesh +
# props) as ONE Wavefront .obj + companion .mtl, with each piece grouped
# under its own named object ("o ...") so Blender's outliner shows a
# sensible hierarchy (Terrain / Foliage_Tree / Road_1 / Road_1_Fence ...)
# after import. Foliage/prop instances are fully baked (rotation, scale,
# world position all applied to the vertices) rather than referencing an
# external prototype, since .obj has no instancing concept of its own -
# this keeps the exported file self-contained and Blender-import-safe.
#
# Materials are flat colors (matching what's drawn in the live viewport
# for foliage/props) except the terrain and roads, whose procedural/
# imported textures are baked out to companion .png files sitting next
# to the .obj/.mtl, so the same look carries over into Blender.

def _obj_mesh_block(name, verts, normals, faces_with_uv=None, uvs=None, material=None):
    """Builds the "o name" / v / vn / (vt) / usemtl / f lines for one mesh
    object. `faces_with_uv` is a list of tuples of (vertex_indices,
    normal_indices, uv_indices_or_None); vertex/normal indices are LOCAL
    to this block (0-based) and get offset to the running global OBJ
    index by the caller."""
    lines = ["o {}".format(name)]
    for v in verts:
        lines.append("v {:.6f} {:.6f} {:.6f}".format(v[0], v[1], v[2]))
    for n in normals:
        lines.append("vn {:.6f} {:.6f} {:.6f}".format(n[0], n[1], n[2]))
    has_uv = uvs is not None and len(uvs) > 0
    if has_uv:
        for uv in uvs:
            lines.append("vt {:.6f} {:.6f}".format(uv[0], uv[1]))
    if material:
        lines.append("usemtl {}".format(material))
    for face in faces_with_uv:
        vidx, nidx = face[0], face[1]
        if has_uv and len(face) > 2 and face[2] is not None:
            tidx = face[2]
            tokens = [
                "{}/{}/{}".format(vidx[k] + 1, tidx[k] + 1, nidx[k] + 1)
                for k in range(len(vidx))
            ]
        else:
            tokens = ["{}//{}".format(vidx[k] + 1, nidx[k] + 1) for k in range(len(vidx))]
        lines.append("f " + " ".join(tokens))
    return lines, len(verts), len(normals), (len(uvs) if has_uv else 0)


def _save_texture_png(gl_tex_id, out_path, size=256):
    """Reads a GL texture back into an image and writes it to disk (used
    to bake terrain/road textures out alongside the exported .obj so the
    material actually shows up when the file is opened in Blender).
    Requires Pillow; if it's not installed the caller just skips writing
    an image and the .mtl falls back to a flat average color instead."""
    from PIL import Image
    ...
    raw = glGetTexImage(GL_TEXTURE_2D, 0, GL_RGB, GL_UNSIGNED_BYTE)
    glBindTexture(GL_TEXTURE_2D, 0)
    arr = np.frombuffer(raw, dtype="uint8").reshape((h, w, 3))
    arr = np.flipud(arr)  # GL texture row 0 is the bottom; image row 0 is the top
    Image.fromarray(arr, "RGB").save(out_path)

def bake_terrain_blend_texture(terrain, tex_ids, weights, tile_scale, size=512):
    """Bakes the terrain's 4-layer splat blend (grass/dirt/rock/sand) into
    ONE RGB image, sampled the same way the live GL shader samples it:
    weights are bilinearly interpolated across the vertex grid (matching
    the shader's per-fragment `varying` interpolation), then used to
    blend the four tiled layer textures at that world position. Exporting
    this single baked image - instead of assigning one dominant layer per
    triangle - is what makes the mesh show smooth gradient blending in
    Blender instead of hard per-triangle seams."""
    layer_keys = ("grass", "dirt", "rock", "sand")
    layer_arrays = []
    for key in layer_keys:
        tex_id = tex_ids.get(key)
        if tex_id is None:
            layer_arrays.append(np.zeros((4, 4, 3), dtype="float32"))
            continue
        ...
        raw = glGetTexImage(GL_TEXTURE_2D, 0, GL_RGB, GL_UNSIGNED_BYTE)
        glBindTexture(GL_TEXTURE_2D, 0)
        arr = np.frombuffer(raw, dtype="uint8").reshape((th, tw, 3)).astype("float32") / 255.0
    ...
    zs = np.linspace(z_min, z_max, size)
    world_x, world_z = np.meshgrid(xs, zs)  # (size, size); row -> z, col -> x

    # -- bilinearly interpolate the per-vertex weights across the terrain
    # grid, exactly like the shader's varying interpolation across a face --
    gx, gz = terrain.world_to_grid(world_x, world_z)
    gx = np.clip(gx, 0.0, terrain.width - 1.0 - 1e-6)
    gz = np.clip(gz, 0.0, terrain.height - 1.0 - 1e-6)
    x0 = gx.astype(int)
    z0 = gz.astype(int)
    x1 = np.clip(x0 + 1, 0, terrain.width - 1)
    z1 = np.clip(z0 + 1, 0, terrain.height - 1)
    tx = (gx - x0)[..., None]
    tz = (gz - z0)[..., None]

    w_grid = weights.reshape(terrain.height, terrain.width, 4)
    w00, w10 = w_grid[z0, x0], w_grid[z0, x1]
    w01, w11 = w_grid[z1, x0], w_grid[z1, x1]
    w0 = w00 * (1 - tx) + w10 * tx
    w1 = w01 * (1 - tx) + w11 * tx
    w = w0 * (1 - tz) + w1 * tz
    wsum = w.sum(axis=-1, keepdims=True)
    ...
    # -- sample each tiled layer texture at this world position (bilinear,
    # wrapping like GL_REPEAT), same uv formula as the shader --
    albedo = np.zeros((size, size, 3), dtype="float32")
    for i, arr in enumerate(layer_arrays):
        th, tw = arr.shape[0], arr.shape[1]
        u = (world_x / max(tile_scale, 1e-6)) % 1.0
        v = (world_z / max(tile_scale, 1e-6)) % 1.0
        fx, fz = u * tw, v * th
        ix0 = np.floor(fx).astype(int) % tw
        iz0 = np.floor(fz).astype(int) % th
        ix1 = (ix0 + 1) % tw
        iz1 = (iz0 + 1) % th
        ftx = (fx - np.floor(fx))[..., None]
        ftz = (fz - np.floor(fz))[..., None]
        s00, s10 = arr[iz0, ix0], arr[iz0, ix1]
        s01, s11 = arr[iz1, ix0], arr[iz1, ix1]
        s0 = s00 * (1 - ftx) + s10 * ftx
        s1 = s01 * (1 - ftx) + s11 * ftx
        albedo += (s0 * (1 - ftz) + s1 * ftz) * w[..., i:i + 1]

    albedo = np.clip(albedo, 0.0, 1.0)
    # flip vertically before saving: most OBJ/Blender importers treat
    # vt v=0 as the BOTTOM of the image, so row0 (top) should be z_max
    # and the last row (bottom) should be z_min to match v=(z-z_min)/z_span
    return (np.flipud(albedo) * 255).astype("uint8")

def export_scene_to_obj(path, terrain, terrain_tex_ids, terrain_weights, terrain_tile_scale,
                         foliage, foliage_display_lists,
                         roads, road_texture_presets, road_prop_lists,
                         export_textures=True, progress_cb=None):
    """Writes the whole scene (terrain + foliage + roads/props) to a
    single .obj/.mtl pair that Blender can import directly (File > Import
    > Wavefront (.obj)). `path` is the target .obj path; the .mtl and any
    baked texture .png files are written next to it using the same base
    name. Returns a short status string on success; raises on failure so
    the caller can show the error in the UI.

    Since GL display lists (used for the built-in foliage/prop meshes)
    can't be introspected for their vertex data, this rebuilds each
    foliage/prop instance's geometry directly from the same generator
    functions used to build the display lists in the first place, then
    bakes each instance's transform (position/rotation/scale) into the
    exported vertices."""
    ...
    obj_lines = ["# Exported from Terrain Editor", "mtllib " + mtl_filename]
    mtl_lines = []
    materials_written = set()
    ...
    def add_material_flat(name, color):
        if name in materials_written:
            return
        materials_written.add(name)
        mtl_lines.extend([
            "newmtl {}".format(name),
            "Kd {:.4f} {:.4f} {:.4f}".format(color[0], color[1], color[2]),
            "Ka 0.05 0.05 0.05",
            "Ks 0.0 0.0 0.0",
            "d 1.0",
            "illum 1",
    ...
    def add_material_textured(name, gl_tex_id):
        if name in materials_written:
            return
        materials_written.add(name)
        tex_filename = None
        if export_textures and gl_tex_id is not None:
            try:
                tex_filename = "{}_{}.png".format(base_name, name)
                _save_texture_png(gl_tex_id, os.path.join(base_dir, tex_filename))
            except Exception:
                tex_filename = None
        mtl_lines.append("newmtl {}".format(name))
        mtl_lines.append("Kd 1.0 1.0 1.0")
        mtl_lines.append("Ka 0.05 0.05 0.05")
        mtl_lines.append("Ks 0.0 0.0 0.0")
        mtl_lines.append("d 1.0")
        mtl_lines.append("illum 1")
        if tex_filename:
            mtl_lines.append("map_Kd " + tex_filename)
        mtl_lines.append("")

    # -- 1. terrain -----------------------------------------------------
    report("Exporting terrain...")
    verts = terrain.vertices
    normals = terrain.normals
    tris = terrain.tris

    vx, vz = verts[:, 0], verts[:, 2]
    x_min, x_max = float(vx.min()), float(vx.max())
    z_min, z_max = float(vz.min()), float(vz.max())
    x_span = max(x_max - x_min, 1e-6)
    z_span = max(z_max - z_min, 1e-6)
    # planar UV matching the bake's sampling grid exactly, so the smooth
    # per-vertex blend baked into the texture lines up with the mesh
    uvs = np.stack([(vx - x_min) / x_span, (vz - z_min) / z_span], axis=1)

    mat_name = "terrain_blend"
    if mat_name not in materials_written:
        materials_written.add(mat_name)
        tex_filename = None
        if export_textures:
            try:
                baked = bake_terrain_blend_texture(
                    terrain, terrain_tex_ids, terrain_weights, terrain_tile_scale
                )
                tex_filename = "{}_terrain_blend.png".format(base_name)
                from PIL import Image
                Image.fromarray(baked, "RGB").save(os.path.join(base_dir, tex_filename))
            except Exception as e:
                print("Terrain texture bake failed:", e)
                tex_filename = None

        mtl_lines.append("newmtl {}".format(mat_name))
        mtl_lines.append("Kd 1.0 1.0 1.0")
        mtl_lines.append("Ka 0.05 0.05 0.05")
        mtl_lines.append("Ks 0.0 0.0 0.0")
        mtl_lines.append("d 1.0")
    ...
    obj_lines.append("o Terrain")
    for v in verts:
        obj_lines.append("v {:.6f} {:.6f} {:.6f}".format(v[0], v[1], v[2]))
    for n in normals:
        obj_lines.append("vn {:.6f} {:.6f} {:.6f}".format(n[0], n[1], n[2]))
    for uv in uvs:
        obj_lines.append("vt {:.6f} {:.6f}".format(uv[0], uv[1]))
    obj_lines.append("usemtl " + mat_name)
    for a, b, c in tris:
        obj_lines.append(
            "f {0}/{0}/{0} {1}/{1}/{1} {2}/{2}/{2}".format(
                a + 1 + v_offset, b + 1 + v_offset, c + 1 + v_offset
            )
        )
    v_offset += len(verts)
    ...
    # -- 2. foliage -------------------------------------------------------
    # rebuild each prototype's raw (untransformed) geometry once per type,
    # then bake every instance's transform into its own copy of that mesh
    report("Exporting foliage...")
    if len(foliage):
        proto_cache = {}
        FOLIAGE_COLORS = {
            FOLIAGE_TREE: [(0.36, 0.24, 0.14), (0.15, 0.40, 0.17)],
            FOLIAGE_BUSH: [(0.20, 0.44, 0.19)],
            FOLIAGE_GRASS: [(0.36, 0.62, 0.24)],
            FOLIAGE_LEAVES: [(0.55, 0.35, 0.12)],
            FOLIAGE_MUD: [(0.30, 0.20, 0.12)],
            FOLIAGE_SAND: [(0.76, 0.70, 0.50)],
            FOLIAGE_ROCK: [(0.46, 0.45, 0.43)],
            FOLIAGE_FLOWER: [(0.30, 0.55, 0.20), (0.85, 0.25, 0.35)],
            FOLIAGE_DEADTREE: [(0.32, 0.27, 0.22)],
        }

        def get_prototype(ftype):
            """Returns (verts_local, normals_local, tris) for one foliage
            prototype, computed directly (not via the GL display list) so
            it can be exported without an active GL context read-back."""
            if ftype in proto_cache:
                return proto_cache[ftype]
            geo = _build_foliage_prototype_geometry(ftype)
            proto_cache[ftype] = geo
            return geo

        for ftype in FOLIAGE_RULES:
            pass  # rules indexed separately below by int type id

        for ftype_id in sorted(set(int(t) for t in foliage.types)):
            if ftype_id < len(FOLIAGE_RULES):
                type_name = FOLIAGE_RULES[ftype_id]["name"].replace(" ", "_")
            else:
                type_name = "custom_{}".format(ftype_id)
            mat_name = "foliage_{}".format(type_name)
            base_color = FOLIAGE_COLORS.get(ftype_id, [(0.4, 0.4, 0.4)])[0]
            add_material_flat(mat_name, base_color)

            try:
                proto_v, proto_n, proto_tris = get_prototype(ftype_id)
            except Exception:
                continue  # custom-imported types (no python-side prototype) skipped

            idxs = np.nonzero(foliage.types == ftype_id)[0]
            obj_lines.append("o Foliage_{}".format(type_name.capitalize()))
            inst_v_count = 0
            inst_vn_count = 0
            all_faces = []
            for inst_i in idxs:
                pos = foliage.positions[inst_i]
                rot = float(foliage.rotations[inst_i])
                scale = float(foliage.scales[inst_i])
                cos_r, sin_r = math.cos(rot), math.sin(rot)
                # bake: scale -> yaw rotate (Y axis) -> translate, matching
                # the live glScalef/glRotatef/glTranslatef order in draw_foliage
                rv = proto_v * scale
                rx = rv[:, 0] * cos_r + rv[:, 2] * sin_r
                rz = -rv[:, 0] * sin_r + rv[:, 2] * cos_r
                world_v = np.stack([rx + pos[0], rv[:, 1] + pos[1], rz + pos[2]], axis=1)
                nx = proto_n[:, 0] * cos_r + proto_n[:, 2] * sin_r
                nz = -proto_n[:, 0] * sin_r + proto_n[:, 2] * cos_r
                world_n = np.stack([nx, proto_n[:, 1], nz], axis=1)

                for v in world_v:
                    obj_lines.append("v {:.6f} {:.6f} {:.6f}".format(v[0], v[1], v[2]))
                for n in world_n:
                    obj_lines.append("vn {:.6f} {:.6f} {:.6f}".format(n[0], n[1], n[2]))
                for a, b, c in proto_tris:
                    all_faces.append((
                        a + 1 + v_offset + inst_v_count,
                        b + 1 + v_offset + inst_v_count,
                        c + 1 + v_offset + inst_v_count,
                        a + 1 + vn_offset + inst_vn_count,
                        b + 1 + vn_offset + inst_vn_count,
                        c + 1 + vn_offset + inst_vn_count,
                    ))
                inst_v_count += len(world_v)
                inst_vn_count += len(world_n)

            obj_lines.append("usemtl " + mat_name)
            for a, b, c, na, nb, nc in all_faces:
                obj_lines.append("f {}//{} {}//{} {}//{}".format(a, na, b, nb, c, nc))
            v_offset += inst_v_count
            vn_offset += inst_vn_count

    # -- 3. roads (mesh + props) -----------------------------------------
    report("Exporting roads...")
    # was: ROAD_PROP_COLORS = {"fence": (0.42, 0.30, 0.16), "lamp": (0.20, 0.20, 0.22)}
    ROAD_PROP_PART_COLORS = {("fence", "rod"): (0.42, 0.30, 0.16),("lamp", "rod"): (0.20, 0.20, 0.22),("lamp", "light"): (0.95, 0.85, 0.45),}
    for ri, road in enumerate(roads):
        mesh = road.get_mesh()
        road_name = road.name.replace(" ", "_") or "Road_{}".format(ri + 1)
        if mesh is not None:
            rverts, rnormals, ruvs, rtris = mesh
            tex_id = road_texture_presets.get(road.texture_key)
            mat_name = "road_{}".format(road_name)
            add_material_textured(mat_name, tex_id)

            obj_lines.append("o {}".format(road_name))
            for v in rverts:
                obj_lines.append("v {:.6f} {:.6f} {:.6f}".format(v[0], v[1], v[2]))
            for n in rnormals:
                obj_lines.append("vn {:.6f} {:.6f} {:.6f}".format(n[0], n[1], n[2]))
            ...
            v_offset += len(rverts)
            vn_offset += len(rnormals)
            vt_offset += len(ruvs)

            # -- modular props along this road (fence/lamp/custom) --
            instances = road.get_prop_instances()
            if instances:
                by_type = {}
                for pos, yaw, side, prop_type in instances:
                    by_type.setdefault(prop_type, []).append((pos, yaw))
                for prop_type, inst_list in by_type.items():
                    try:
                        proto_parts = _build_road_prop_prototype_geometry(prop_type)
                    except Exception:
                        continue  # custom-imported prop types skipped (no python-side prototype)

                    for part_name, (proto_v, proto_n, proto_tris) in proto_parts.items():
                        mat_name = "prop_{}_{}".format(prop_type, part_name)
                        color = ROAD_PROP_PART_COLORS.get((prop_type, part_name), (0.4, 0.4, 0.4))
                        add_material_flat(mat_name, color)

                        obj_lines.append("o {}_{}_{}".format(
                            road_name, prop_type.capitalize(), part_name.capitalize()
                        ))
                        inst_v_count = 0
                        inst_vn_count = 0
                        all_faces = []
                        for pos, yaw in inst_list:
                            cos_r, sin_r = math.cos(yaw), math.sin(yaw)
                            rx = proto_v[:, 0] * cos_r + proto_v[:, 2] * sin_r
                            rz = -proto_v[:, 0] * sin_r + proto_v[:, 2] * cos_r
                            world_v = np.stack([rx + pos[0], proto_v[:, 1] + pos[1], rz + pos[2]], axis=1)
                            nx = proto_n[:, 0] * cos_r + proto_n[:, 2] * sin_r
                            nz = -proto_n[:, 0] * sin_r + proto_n[:, 2] * cos_r
                            world_n = np.stack([nx, proto_n[:, 1], nz], axis=1)
    ...
    return "Exported scene to '{}' (+ .mtl{}).".format(
        os.path.basename(path), " + baked textures" if export_textures else ""
    )


# -- CPU-side (non-GL) geometry builders for export ------------------------
# Mirror the _build_*_list()/prop functions above vertex-for-vertex, but
# return plain numpy arrays instead of compiling a GL display list, so
# export_scene_to_obj() can bake instance transforms without needing to
# read geometry back out of the GPU.

def _tris_to_arrays(tri_verts, tri_normals):
    """tri_verts/tri_normals: lists of 3-tuples-of-xyz per triangle vertex
    (i.e. len == 3 * num_triangles). Deduplicates nothing - simplest
    correct representation, one unique vert per triangle corner."""
    verts = np.array(tri_verts, dtype="float64")
    normals = np.array(tri_normals, dtype="float64")
    tris = np.arange(len(verts)).reshape(-1, 3)
    return verts, normals, tris


def _build_foliage_prototype_geometry(ftype):
    tv, tn = [], []

    def push_tri(p0, p1, p2, n0, n1, n2):
        tv.extend([p0, p1, p2])
        tn.extend([n0, n1, n2])

    if ftype == FOLIAGE_TREE:
        trunk_segments, trunk_h, trunk_r = 6, 0.55, 0.055
        for i in range(trunk_segments):
            a0 = 2 * math.pi * i / trunk_segments
            a1 = 2 * math.pi * (i + 1) / trunk_segments
            x0, z0 = math.cos(a0) * trunk_r, math.sin(a0) * trunk_r
            ...
            push_tri((x0, 0.0, z0), (x0, trunk_h, z0), (x1, 0.0, z1), n0, n0, n1)
            push_tri((x1, 0.0, z1), (x0, trunk_h, z0), (x1, trunk_h, z1), n1, n0, n1)
        cone_segments = 8
        layers = [(0.55, trunk_h, 0.65), (0.42, trunk_h + 0.42, 0.55), (0.28, trunk_h + 0.78, 0.42)]
        for radius, base_y, cone_h in layers:
            tip = (0.0, base_y + cone_h, 0.0)
            for i in range(cone_segments):
                ...
                nz = math.sin((a0 + a1) / 2.0)
                n = (nx, 0.5, nz)
                push_tri((x0, base_y, z0), (x1, base_y, z1), tip, n, n, n)
    elif ftype == FOLIAGE_BUSH:
        radius = 0.28
        for p0, p1, p2 in _sphere_triangles(5, 8):
            pts, ns = [], []
            for p in (p0, p1, p2):
                pts.append((p[0] * radius, p[1] * radius, p[2] * radius))
                ns.append((p[0], p[1] - 1.0, p[2]))
            push_tri(pts[0], pts[1], pts[2], ns[0], ns[1], ns[2])
    elif ftype == FOLIAGE_GRASS:
        w, h = 0.28, 0.45
        for angle in (0.0, math.pi / 2.0):
            cx, cz = math.cos(angle) * w / 2.0, math.sin(angle) * w / 2.0
            p0, p1, p2, p3 = (-cx, 0.0, -cz), (cx, 0.0, cz), (cx, h, cz), (-cx, h, -cz)
            n = (0.0, 1.0, 0.0)
            push_tri(p0, p1, p2, n, n, n)
            push_tri(p0, p2, p3, n, n, n)
    elif ftype == FOLIAGE_LEAVES:
        leaf_positions = [(0.0, 0.0), (0.12, 0.08), (-0.10, 0.10), (0.05, -0.12), (-0.13, -0.05), (0.15, -0.02)]
        y = 0.015
        n = (0.0, 1.0, 0.0)
        for lx, lz in leaf_positions:
            size = 0.10
            p0 = (lx, y, lz - size)
            p1 = (lx + size * 0.6, y, lz)
            p2 = (lx, y, lz + size)
            ...
            push_tri(p0, p1, p2, n, n, n)
            push_tri(p0, p2, p3, n, n, n)
    elif ftype == FOLIAGE_MUD:
        segments, radius, y = 12, 0.5, 0.01
        n = (0.0, 1.0, 0.0)
        for i in range(segments):
            a0 = 2 * math.pi * i / segments
            a1 = 2 * math.pi * (i + 1) / segments
            ...
            x0, z0 = math.cos(a0) * r0, math.sin(a0) * r0
            x1, z1 = math.cos(a1) * r1, math.sin(a1) * r1
            push_tri((0.0, y, 0.0), (x0, y, z0), (x1, y, z1), n, n, n)
    elif ftype == FOLIAGE_SAND:
        segments, radius, y = 12, 0.6, 0.008
        n = (0.0, 1.0, 0.0)
        for i in range(segments):
            a0 = 2 * math.pi * i / segments
            a1 = 2 * math.pi * (i + 1) / segments
            ...
            x1, z1 = math.cos(a1) * r1, math.sin(a1) * r1
            push_tri((0.0, y, 0.0), (x0, y, z0), (x1, y, z1), n, n, n)
    elif ftype == FOLIAGE_ROCK:
        sx, sy, sz = 0.36, 0.24, 0.32
        for p0, p1, p2 in _sphere_triangles(4, 6):
            pts, ns = [], []
            for p in (p0, p1, p2):
                pts.append((p[0] * sx, p[1] * sy, p[2] * sz))
                ns.append((p[0], p[1] - 1.0, p[2]))
            push_tri(pts[0], pts[1], pts[2], ns[0], ns[1], ns[2])
        for ox, oz, s in [(0.32, 0.12, 0.5), (-0.22, -0.28, 0.4)]:
            for p0, p1, p2 in _sphere_triangles(3, 5):
                pts, ns = [], []
                for p in (p0, p1, p2):
                    pts.append((ox + p[0] * sx * s, p[1] * sy * s, oz + p[2] * sz * s))
                    ns.append((p[0], p[1] - 1.0, p[2]))
                push_tri(pts[0], pts[1], pts[2], ns[0], ns[1], ns[2])
    elif ftype == FOLIAGE_FLOWER:
        stem_h, stem_w = 0.22, 0.018
        n = (0.0, 0.0, 1.0)
        push_tri((-stem_w, 0.0, 0.0), (stem_w, 0.0, 0.0), (stem_w, stem_h, 0.0), n, n, n)
        push_tri((-stem_w, 0.0, 0.0), (stem_w, stem_h, 0.0), (-stem_w, stem_h, 0.0), n, n, n)
        bloom_w, petal_h = 0.12, 0.05
        n2 = (0.0, 1.0, 0.0)
        for angle in (0.0, math.pi / 3.0, 2.0 * math.pi / 3.0):
            cx, cz = math.cos(angle) * bloom_w / 2.0, math.sin(angle) * bloom_w / 2.0
            p0, p1 = (-cx, stem_h, -cz), (cx, stem_h, cz)
            p2, p3 = (cx, stem_h + petal_h, cz), (-cx, stem_h + petal_h, -cz)
            push_tri(p0, p1, p2, n2, n2, n2)
            push_tri(p0, p2, p3, n2, n2, n2)
    elif ftype == FOLIAGE_DEADTREE:
        trunk_segments, trunk_h, trunk_r = 6, 0.75, 0.05
        for i in range(trunk_segments):
            a0 = 2 * math.pi * i / trunk_segments
            a1 = 2 * math.pi * (i + 1) / trunk_segments
            x0, z0 = math.cos(a0) * trunk_r, math.sin(a0) * trunk_r
        ...
        for base_y, dx, dz in branch_specs:
            length, r = 0.24, 0.018
            ex, ez, ey = dx * length, dz * length, base_y + 0.10
            push_tri((-r, base_y, 0.0), (r, base_y, 0.0), (ex, ey, ez), n, n, n)
    else:
        raise ValueError("No CPU-side prototype for foliage type {}".format(ftype))

    return _tris_to_arrays(tv, tn)


def _build_road_prop_prototype_geometry(prop_type):
    """Returns a dict {part_name: (verts, normals, tris)} of CPU-side
    prototype geometry for one road-prop type, split into separately
    materialed parts. Mirrors the part colors already used in the live
    GL draw calls (_build_fence_prop_list/_build_lamp_prop_list) so the
    exported .obj shows the same look in Blender: the lamp's dark pole
    and its bright bulb come out as two distinct materials ('rod' and
    'light') instead of one flat-grey mesh."""

    def push_tri(lst_v, lst_n, p0, p1, p2, n0, n1, n2):
        lst_v.extend([p0, p1, p2])
        lst_n.extend([n0, n1, n2])

    if prop_type == "fence":
        tv, tn = [], []
        post_h, post_r, segs = 0.7, 0.045, 6
        for i in range(segs):
            a0 = 2 * math.pi * i / segs
            a1 = 2 * math.pi * (i + 1) / segs
            x0, z0 = math.cos(a0) * post_r, math.sin(a0) * post_r
            ...
            n0 = (math.cos(a0), 0.0, math.sin(a0))
            n1 = (math.cos(a1), 0.0, math.sin(a1))
            push_tri(tv, tn, (x0, 0.0, z0), (x0, post_h, z0), (x1, 0.0, z1), n0, n0, n1)
            push_tri(tv, tn, (x1, 0.0, z1), (x0, post_h, z0), (x1, post_h, z1), n1, n0, n1)
        rail_len, rail_thick = 0.9, 0.04
        n = (0.0, 1.0, 0.0)
        for rail_y in (0.30, 0.55):
            p0 = (-rail_len * 0.5, rail_y, -rail_thick)
            p1 = (rail_len * 0.5, rail_y, -rail_thick)
            p2 = (rail_len * 0.5, rail_y, rail_thick)
            p3 = (-rail_len * 0.5, rail_y, rail_thick)
            push_tri(tv, tn, p0, p1, p2, n, n, n)
            push_tri(tv, tn, p0, p2, p3, n, n, n)
        return {"rod": _tris_to_arrays(tv, tn)}

    elif prop_type == "lamp":
        pole_tv, pole_tn = [], []
        light_tv, light_tn = [], []

        pole_h, pole_r, segs = 2.2, 0.05, 6
        for i in range(segs):
            a0 = 2 * math.pi * i / segs
            a1 = 2 * math.pi * (i + 1) / segs
            x0, z0 = math.cos(a0) * pole_r, math.sin(a0) * pole_r
            ...
            n0 = (math.cos(a0), 0.0, math.sin(a0))
            n1 = (math.cos(a1), 0.0, math.sin(a1))
            push_tri(pole_tv, pole_tn, (x0, 0.0, z0), (x0, pole_h, z0), (x1, 0.0, z1), n0, n0, n1)
            push_tri(pole_tv, pole_tn, (x1, 0.0, z1), (x0, pole_h, z0), (x1, pole_h, z1), n1, n0, n1)

        r = 0.16
        for p0, p1, p2 in _sphere_triangles(4, 6):
            pts, ns = [], []
            for p in (p0, p1, p2):
                pts.append((p[0] * r, pole_h + (p[1] - 1.0) * r, p[2] * r))
                ns.append((p[0], p[1] - 1.0, p[2]))
            push_tri(light_tv, light_tn, pts[0], pts[1], pts[2], ns[0], ns[1], ns[2])

        return {
            "rod": _tris_to_arrays(pole_tv, pole_tn),
            "light": _tris_to_arrays(light_tv, light_tn),
        }
    else:
        raise ValueError("No CPU-side prototype for road prop type '{}'".format(prop_type))
...
def main():
    ...
    # -- scene export (combined .obj/.mtl for Blender) ----------------------
    export_path = os.path.join(default_browse_start_dir(), "terrain_scene.obj")
    export_textures = True
    export_status = ""
    show_export_inapp_browser = False
    export_browse_dir = default_browse_start_dir()
    ...
    while True:
        ...
        imgui.separator()
        if imgui.tree_node("Export Scene (.obj for Blender)"):
            imgui.text_wrapped(
                "Exports the terrain, every foliage instance, and every "
                "road (mesh + fences/lamps/custom props) as ONE combined "
                ".obj + .mtl you can open directly in Blender via "
                "File > Import > Wavefront (.obj). Each piece is baked "
                "into its own named object (Terrain / Foliage_Tree / "
                "Road_1 / Road_1_Fence / ...)."
            )
            imgui.text("Export path:")
            imgui.push_item_width(-1)
            _, export_path = imgui.input_text("##export_path", export_path, 512)
            imgui.pop_item_width()
            if imgui.button("Browse...##exportpath"):
                dialog_worked, picked = browse_for_save_file(
                    "Export scene as", ".obj", [("Wavefront OBJ", "*.obj"), ("All files", "*.*")]
                )
                if dialog_worked:
                    if picked:
                        export_path = picked
                else:
                    show_export_inapp_browser = True
            imgui.same_line()
            if imgui.button("Browse (in-app)##exportpath"):
                show_export_inapp_browser = True
            _, export_textures = imgui.checkbox("Bake textures to .png (needs Pillow)", export_textures)
            if imgui.button("Export Scene"):
                try:
                    if not export_path.lower().endswith(".obj"):
                        export_path = export_path + ".obj"
                    status_holder = {"msg": ""}
                    export_status =  export_scene_to_obj(
                        export_path, terrain, terrain_tex_ids, terrain_textures.weights,
                        terrain_textures.tile_scale,
                        foliage, foliage_display_lists,
                        roads, road_texture_presets, road_prop_lists,
                        export_textures=export_textures,
                        progress_cb=lambda m: status_holder.__setitem__("msg", m),
                    )
                except Exception as e:
                    export_status = "Export failed: {}".format(e)
            if export_status:
                imgui.text_wrapped(export_status)
            imgui.tree_pop()
        ...
        if show_export_inapp_browser:
            still_open, export_browse_dir, picked_dir_entry = draw_inapp_browser(
                export_browse_dir, title="Choose export folder (pick any file to select its folder)",
                extensions=(".obj",)
            )
            show_export_inapp_browser = still_open
            if picked_dir_entry:
                # re-use the browser's own folder as the export destination,
                # keeping the current filename
                export_path = os.path.join(os.path.dirname(picked_dir_entry), os.path.basename(export_path))
                show_export_inapp_browser = False
